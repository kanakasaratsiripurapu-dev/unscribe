...
"""
    ...
    def _build_extraction_prompt(self, email_text: str) -> str:
        ...
        # This is defined in the extraction prompt artifact created earlier
        return f"""You are an expert at extracting subscription information from emails.

Extract the following fields as JSON:
- service_name (required)
- price (required, float)
- currency (default "USD")
- billing_period (one of: monthly, annually, quarterly, one-time)
- next_renewal_date (ISO 8601 format, optional)
- unsubscribe_link (full URL, required)
- subscription_tier (optional)
- confidence (0.0 to 1.0, your confidence in extraction)

Rules:
1. Return ONLY valid JSON, no markdown or explanation
2. Validate that unsubscribe_link is a complete URL
3. If a field cannot be found, use null
4. Normalize service names (e.g., "Netflix Premium" → "Netflix")
