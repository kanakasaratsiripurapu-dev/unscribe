...
def scan_gmail_inbox(self: Task, user_id: str, session_id: str, date_range_years: int = 3):
    ...
    This task:
    1. Fetches emails from Gmail in batches
    2. Applies detection pipeline to each email
    3. Stores detected subscriptions in database
    4. Updates session progress
    """
...
async def _scan_gmail_inbox_async(task: Task, user_id: str, session_id: str, date_range_years: int):
    ...
    async with async_session() as db:
        ...
        try:
            ...
            access_token = await gmail_service.refresh_access_token(
                user.gmail_refresh_token  # This should be decrypted first
            )
            
            # Build Gmail search query
            # Search for emails from last N years
            ...
            while True:
                ...
                if page_token is None:
                    session.total_emails_found = result.get('resultSizeEstimate', 0)
                    await db.commit()
...
def execute_unsubscribe(subscription_id: str, action_id: str, user_access_token: str):
    ...
    Steps:
    1. Attempt automated cancellation
    2. Monitor for confirmation email (7 days)
...
@celery_app.task(name='monitor_cancellation_confirmation')
def monitor_cancellation_confirmation(subscription_id: str, action_id: str, user_access_token: str):
    """
    Task: Monitor inbox for cancellation confirmation
    Runs periodically until confirmation found or timeout
    """
    import asyncio
    return asyncio.run(_monitor_confirmation_async(subscription_id, action_id, user_access_token))


async def _monitor_confirmation_async(subscription_id: str, action_id: str, user_access_token: str):
    """Async implementation of confirmation monitoring"""
    ...
    async with async_session() as db:
        ...
        if not subscription or not action:
            return {"error": "Not found"}
        
        # Check if monitoring period expired
        if datetime.utcnow() > action.monitoring_until:
            action.status = 'failed'
            action.error_message = 'No confirmation email received within 7 days'
            subscription.status = 'active'  # Revert to active
            await db.commit()
            return {"status": "timeout"}
        
        # Extract service domain from sender or unsubscribe link
        service_domain = subscription.service_domain or _extract_domain(subscription.unsubscribe_link)
        
        # Check for confirmation email
        confirmation = await unsubscribe_service.monitor_confirmation(
            service_domain=service_domain,
            user_access_token=user_access_token,
            start_date=action.initiated_at,
            days_to_monitor=settings.CONFIRMATION_MONITORING_DAYS
        )
        
        if confirmation:
            # Confirmation found!
            action.status = 'confirmed'
            action.confirmation_email_id = confirmation['confirmation_email_id']
            action.confirmation_detected_at = datetime.utcnow()
            ...
            logger.info(f"Cancellation confirmed for subscription {subscription_id}")
            return {"status": "confirmed"}
        
        else:
            # No confirmation yet, schedule another check
            celery_app.send_task(
                'monitor_cancellation_confirmation',
                args=[subscription_id, action_id, user_access_token],
